...
def first_join(user_id, first_name, username, code):
    ...
    if len(row) == 0:
        ...
        cursor.execute(sql, (user_id, first_name, f"@{username}", '0', who_invite, datetime.datetime.now(), 'no', 0, 0))
        conn.commit()
...
async def pact(user_id, reply_id):
    await bot.send_message(chat_id=user_id, text=texts.pact, reply_markup=menu.pact(), reply_to_message_id=reply_id)
...
def float_date(string):
    return datetime.datetime.strptime(string, "%Y-%m-%d %H:%M").timestamp()
...
def dealing_status_to_text(status):
    if status in ['prepare', 'clarify']:
        return 'на рассмотрении'
    if status == 'open':
        return 'открыта'
    if status == 'confirm':
        return 'на финальном этапе'
    if status == 'success':
        return 'успешна'
    if status == 'suspend':
        return 'спорна'
